...
class RNNModel:
	...
	def read_data(self):
		"""Llegeix l'arxiu CSV"""
		column_dtypes = {i: int for i in range(1, 7)}
		self.data = pd.read_csv(self.input_file, sep=";", parse_dates=[0], dtype=column_dtypes)
	...
	def set_columns(self):
		...
		self.num_features = [col for col in self.data.columns[1:]]
	...
	def split_data(self):
		...
		X = self.data.drop(columns=self.num_features)
	...
	def generate_future_data(self):
		...
		# Crea un DataFrame buit per a emmagatzemar les combinacions de data
		self.future_data = pd.DataFrame(columns=[self.date_col] + self.num_features)
		
		# Añade las fechas futuras al DataFrame
	...
	def save_predictions(self):
		...
		columns_order = [self.date_col] + self.num_features[:num_features_to_save]  # Corregido aquí
...
if __name__ == '__main__':
	predictor = RNNModel(input_file=".\\dades\\Dades.csv")
	predictor.read_data()
